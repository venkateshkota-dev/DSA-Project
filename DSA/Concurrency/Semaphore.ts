...
export class Semaphore {
   ...
   */
  *acquireGenerator(ownerName: string): Generator<SemaphoreStep, boolean, unknown> {
    yield {
   ...
   */
  *releaseGenerator(ownerName: string): Generator<SemaphoreStep, void, unknown> {
    const ownerIdx = this.activeOwners.indexOf(ownerName);
    if (ownerIdx !== -1) {
      this.activeOwners.splice(ownerIdx, 1);
    }
